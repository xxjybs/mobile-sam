# 脐橙缺陷分割 - 热力图可视化工具

本工具用于可视化和分析脐橙缺陷分割模型的预测结果，生成热力图对比预测掩码与真实掩码。

## 功能特点

- 📊 **差异分析**: 清晰显示预测与真实掩码的差异
  - 🟢 绿色: 正确预测的缺陷 (True Positive)
  - 🔴 红色: 误报 - 错误预测为缺陷 (False Positive)
  - 🟡 黄色: 漏报 - 遗漏的缺陷 (False Negative)
  - ⚫ 黑色: 正确的背景 (True Negative)

- 📈 **评估指标**: 自动计算并显示
  - IoU (交并比)
  - Precision (精确率)
  - Recall (召回率)
  - F1-Score
  - 混淆矩阵统计 (TP, FP, FN, TN)

- 🎨 **多种可视化**: 
  - 原始掩码对比
  - 彩色差异图
  - 热力图 (伪彩色)
  - 误差热力图

## 文件说明

### 1. `visualize_heatmap_simple.py` (推荐使用)
简化版工具，无需加载模型，直接对比两个掩码图像。

**特点**:
- ✅ 简单易用，无需模型权重
- ✅ 快速生成可视化结果
- ✅ 适合大多数使用场景

### 2. `visualize_heatmap.py` (高级版)
完整版工具，可选加载模型生成预测置信度图。

**特点**:
- 支持加载原始图像
- 可计算预测置信度热力图
- 更详细的可视化选项

## 快速开始

### 最简单的使用方式

如果您已经运行了训练脚本并生成了预测结果:

```bash
# 使用默认路径可视化 398.png
python visualize_heatmap_simple.py
```

这将:
- 读取预测: `./save/mobile_sam_adapter/vit_change_2/pred/398.png`
- 读取真实: `./data/orange/masks/398.png`
- 输出热力图: `./defect_heatmap_398.png`

### 指定其他图像

```bash
# 可视化其他图像，例如 123.png
python visualize_heatmap_simple.py --img_name 123
```

### 完全自定义路径

```bash
python visualize_heatmap_simple.py \
    --pred_path ./save/mobile_sam_adapter/vit_change_2/pred/398.png \
    --gt_path ./data/orange/masks/398.png \
    --output_path ./my_custom_heatmap.png
```

### 自定义默认目录

```bash
python visualize_heatmap_simple.py \
    --img_name 398 \
    --pred_dir ./save/mobile_sam_adapter/my_experiment/pred \
    --gt_dir ./data/orange/masks
```

## 高级用法 (使用完整版)

如果您想要生成预测置信度热力图，使用 `visualize_heatmap.py`:

```bash
python visualize_heatmap.py \
    --pred_path ./save/mobile_sam_adapter/vit_change_2/pred/398.png \
    --gt_path ./data/orange/masks/398.png \
    --output_path ./defect_heatmap_with_confidence_398.png \
    --model_name mobile_sam_adapter \
    --ckpt_path ./save/mobile_sam_adapter/vit_change_2/mobile_sam_adapter_best.pth \
    --img_name 398 \
    --img_size 256
```

**参数说明**:
- `--model_name`: 模型名称 (例如 `mobile_sam_adapter`)
- `--ckpt_path`: 模型权重文件路径
- `--img_size`: 输入图像尺寸 (根据训练时的设置)
- `--data_dir`: 数据集目录 (包含 `.npy` 文件)

## 批量处理多张图像

创建一个简单的批处理脚本:

```bash
#!/bin/bash
# batch_visualize.sh

for img in 398 399 400 401 402; do
    echo "Processing image $img..."
    python visualize_heatmap_simple.py --img_name $img
done
```

或使用 Python:

```python
import subprocess
import os

img_names = ['398', '399', '400', '401', '402']

for img_name in img_names:
    print(f"Processing {img_name}...")
    subprocess.run([
        'python', 'visualize_heatmap_simple.py',
        '--img_name', img_name
    ])
```

## 输出示例

运行后会在控制台显示详细统计信息:

```
============================================================
✅ 热力图已保存: ./defect_heatmap_398.png
============================================================
📊 评估指标:
   IoU (交并比):     0.7845
   Precision (精确率): 0.8234
   Recall (召回率):   0.8912
   F1-Score:         0.8560

📈 混淆矩阵统计:
   True Positive  (TP - 正确检测):   45,678 像素
   False Positive (FP - 误报):        9,234 像素
   False Negative (FN - 漏报):        5,467 像素
   True Negative  (TN - 正确背景): 205,123 像素
============================================================
```

生成的热力图包含 6 个子图:
1. **预测掩码** (灰度)
2. **真实掩码** (灰度)
3. **差异分析图** (彩色标注)
4. **预测掩码热力图** (伪彩色)
5. **真实掩码热力图** (伪彩色)
6. **误差热力图** (差异强度)

## 依赖要求

```bash
pip install numpy matplotlib pillow torch scipy
```

如果使用完整版工具，还需要项目中的模型定义。

## 常见问题

### Q: 提示 "文件不存在" 怎么办?

A: 请检查:
1. 是否已运行 `train.py` 生成预测结果
2. 路径是否正确 (注意相对路径 vs 绝对路径)
3. 文件名是否正确 (包括扩展名 `.png`)

### Q: 热力图显示效果不理想?

A: 可以尝试:
1. 检查预测掩码和真实掩码是否都是二值图像 (0 和 255)
2. 确认两个掩码的尺寸是否匹配
3. 使用完整版工具生成置信度图以获得更多信息

### Q: 如何在没有 GUI 的服务器上运行?

A: 脚本已经设置为使用非交互式后端 (`matplotlib.use('Agg')`)，可以在无 GUI 环境中运行。

### Q: 中文标题显示为方框?

A: 这是字体问题。可以修改脚本中的字体设置:
```python
# 在脚本开头添加
import matplotlib.pyplot as plt
plt.rcParams['font.sans-serif'] = ['SimHei', 'DejaVu Sans']  # 使用黑体
plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示问题
```

或者简单地将标题改为英文。

## 技术支持

如有问题或建议，请在项目中提 Issue。

## 许可证

遵循项目主仓库的许可证。
