...
from models.sam2_adapter.sam2.modeling.backbones.hieradet import Hiera
...
class SAM2_Adapter_Light(nn.Module):
    def __init__(self, inp_size=1024,
        ...
        self.image_encoder = ImageEncoder(
            trunk=Hiera(
                in_chans=3,
                embed_dims=[64, 128, 256, 512],  # 通道缩小[64, 128, 256, 512]
                img_size=1024,
                patch_size=4,
                embed_dim=64,
                num_heads=1,
                stages=(1, 1, 3, 1),  # 每层 block 数减少
                window_spec=(8, 4, 8, 4),  # 窗口 attention 简化
                global_att_blocks=(3,),  # 只在 stage3 放全局 attn
            ),
            neck=FpnNeck(backbone_channel_list=[512, 256, 128, 64]),
        )
        self._bb_feat_sizes = [
            (256, 256),
            (128, 128),
    ...
    def forward(self, input, distill=False):
        bs = input.shape[0]
        ...
        backbone_features_for_distill = [f.clone() for f in self.features["backbone_fpn"]]
        ...
        if distill:
            return masks, backbone_features_for_distill
        else:
            return masks
