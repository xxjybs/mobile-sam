...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # 根据img_size动态计算feature_size
        patch_size = 4  # patch embedding后的尺寸是img_size // 4
    ...
    def forward_features(self, x):
        ...
        for i in range(start_i, len(self.layers)):
            ...
            prompt = self.prompt_generator.init_prompt(x_reshape, prohandcrafteds[i]+branch_out_reshape, i+1)
            x = layer(x, self.feature_size[i], self.prompt_generator, prompt, block_num=i+1)
        B,_,C=x.size()
        # 根据img_size动态计算最终特征图尺寸
        final_size = self.img_size // 16  # 256//16=16, 1024//16=64
