...
class mobilenetv3_branch(nn.Module):
    def __init__(self, embed_dims=[4,8,10,20]):
        super(mobilenetv3_branch, self).__init__()
        self.blocks = []
        block = nn.Sequential(
            nn.Conv2d(3, 12,5, 2, 2, bias=False),
            nn.BatchNorm2d(12),
            Down_Sample(12, embed_dims[0], kernel_size=3, stride=1, padding=1),
            MobileBottleneck(embed_dims[0], embed_dims[0], 3, 1, 2*embed_dims[0],),
        ).cuda()
        self.blocks.append(block)
        for i in range(len(embed_dims) - 1):
            if i < 2:
                block = nn.Sequential(
                    Down_Sample(embed_dims[i], embed_dims[i + 1], kernel_size=3, stride=1, padding=1),
                    MobileBottleneck(embed_dims[i + 1], embed_dims[i + 1], 3, 1, 2 * embed_dims[i + 1]),
                ).cuda()
            else:
                block = MobileBottleneck(embed_dims[i], embed_dims[i + 1], 3, 1, 2 * embed_dims[i + 1]).cuda()
            self.blocks.append(block)
    ...
    def forward(self, x):
        ...
        B,C,H,W = out1.size()
        out1 = out1.permute(0,2,3,1).view(B, H*W, C)
        B,C,H,W = out2.size()
        out2 = out2.permute(0,2,3,1).view(B, H*W, C)
        B,C,H,W = out3.size()
        out3 = out3.permute(0,2,3,1).view(B, H*W, C)
        B,C,H,W = out4.size()
        out4 = out4.permute(0,2,3,1).view(B, H*W, C)
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        self.branch_mobile_v3 = mobilenetv3_branch()
    ...
    def forward_features(self, x):
        ...
        branch_features = self.branch_mobile_v3(inp)
        ...
        for i in range(start_i, len(self.layers)):
            ...
            prompt = self.prompt_generator.init_prompt(x_reshape, prohandcrafteds[i]+branch_features[i], i + 1)
